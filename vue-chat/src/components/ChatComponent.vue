<template>
    <div class="container">
        <div class="row">
            <div class="col-md-2">
                <img v-bind:src="this.$github_url + user.photo" class="rounded-circle mx-auto" style="width: 25%;" />
        ...
        computed: {
            ws: function() {
                ...
                console.log(this.token)
                    return new WebSocket("ws://127.0.0.1:8091/api/chat/websocket/"+this.user.username);
				},
        },
        methods:{
            sendMessage(){
                if(this.content.trim() != ''){
                    ...
                    const json = {"toName": this.to, "fromAvatar": this.user.photo,"content": this.content};
                    this.content = '';
            ...
            showChat(toName){
                ...
                if(this.records == null){
                    this.$axios.post("http://127.0.0.1:8091/api/chat/records", {"fromName": this.user.username, "toName": this.to},{
        ...
        mounted(){
            ...
            this.ws.onmessage = (event) => {
              ...
              if(data.isListUsers){
                ...
                if(newrecord.toName == "all"){
                    oldrecords = JSON.parse(window.sessionStorage.getItem("all"));
                    if(oldrecords == null)
                        oldrecords = [];
                    ...
                    if(this.to != "all"){
                        const oldValue = this.onlines.get("all");
                        this.onlines.set("all", oldValue + 1);
                    }
